...
Context::Context()
{
  ...
  if (err != SCE_OK)
    throw std::runtime_error("Error while creating render target");
  ...
  for (int i = 0; i < DISPLAY_BUFFER_COUNT; i++) {
    ...
    if (err != SCE_OK)
      throw std::runtime_error("Error color surface init");
    
    err = sceGxmSyncObjectCreate(&_displayBufferSync[i]);
    debugNetPrintf(INFO, (char *)"sceGxmSyncObjectCreate : %#x\n", err);
    if (err != SCE_OK)
      throw std::runtime_error("Error sync object create");
  }
  ...
  if (err != SCE_OK)
    throw std::runtime_error("Error stencil surface init");
...
Context::~Context()
{
  debugNetPrintf(INFO, "Destroying gxm context...");
